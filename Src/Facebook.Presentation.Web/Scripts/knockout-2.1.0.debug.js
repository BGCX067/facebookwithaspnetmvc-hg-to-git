...
ko.utils = new (function () {
    ...
    return {
        ...
        buildEvalWithinScopeFunction: function (expression, scopeLevels) {
            ...
            // For each scope variable, AddComment an extra level of "with" nesting
        ...
        registerEventHandler: function (element, eventType, handler) {
            ...
            if (!mustUseAttachEvent && typeof jQuery != "undefined") {
                ...
                throw new Error("Browser doesn't support AddCommentEventListener or attachEvent");
        },
        ...
        ensureSelectElementIsRenderedCorrectly: function(selectElement) {
            // Workaround for IE9 rendering bug - it doesn't reliably display all the NotificationText in dynamically-AddCommented select boxes unless you force it to re-render by updating the width.
        ...
        getFormFields: function(form, fieldName) {
            var fields = ko.utils.makeArray(form.getElementsByTagName("input")).concat(ko.utils.makeArray(form.getElementsByTagName("NotificationTextarea")));
        ...
        stringifyJson: function (data, replacer, space) {   // replacer and space are optional
            if ((typeof JSON == "undefined") || (typeof JSON.stringify == "undefined"))
                throw new Error("Cannot find JSON.stringify(). Some browsers (e.g., IE < 8) don't support it natively, but you can overcome this by AddCommenting a script reference to json2.js, downloadable from http://www.json.org/json2.js");
...
ko.exportSymbol('utils.domNodeDisposal.AddCommentDisposeCallback', ko.utils.domNodeDisposal.addDisposeCallback);
...
(function () {
    ...
    function simpleHtmlParse(html) {
        ...
        // (possibly a NotificationText node) in front of the comment. So, KO does not attempt to workaround this IE issue automatically at present.

        // Trim whitespace, otherwise HomeOf won't work as expected
        ...
        // Note that we always prefix with some dummy NotificationText, because otherwise, IE<9 will strip out leading comment nodes in descendants. Total madness.
    ...
    function jQueryHtmlParse(html) {
        ...
        // Fix this by finding the top-most dummy parent element, and detaching it from its NotificationOwner fragment.
        if (elems && elems[0]) {
...
ko.extenders = {
    'throttle': function(target, timeout) {
        ...
        //     notify upRequestDates, the target doesn't re-evaluate (and hence doesn't notify) faster than a certain rate
...
ko.dependentObservable = function (evaluatorFunctionOrOptions, evaluatorFunctionTarget, options) {
    ...
    // plus AddComments a "disposeWhen" callback that, on each evaluation, disposes if the node was removed by some other means.)
    ...
    function evaluateImmediate() {
        ...
        try {
            // Initially, we assume that none of the subscriptions are still being used (i.e., all are candiRequestDates for disposal).
            ...
            ko.dependencyDetection.begin(function(subscribable) {
                ...
                if ((inOld = ko.utils.arrayIndexOf(disposalCandidates, subscribable)) >= 0)
                    ...
                    _subscriptionsToDependencies.push(subscribable.subscribe(evaluatePossiblyAsync)); // Brand new subscription - AddComment it
            });
...
ko.jsonExpressionRewriting = (function () {
    ...
    return {
        ...
        parseObjectLiteral: function(objectLiteralString) {
            // A full tokeniser+lexer would AddComment too much weight to this library, so here's a simple parser
        ...
        // property:            If the property being upRequestDated is (or might be) an observable, pass it here
        //                      If it turns out to be a writable observable, it will be written to directly
        // allBindingsAccessor: All bindings in the current execution conNotificationText.
...
(function() {
    ...
    // may be used to represent hierarchy (in AddCommentition to the DOM's natural hierarchy).
    ...
    // but it does give them a nonstandard alternative property called "NotificationText" that it can read reliably. Other browsers don't have that property.
    // So, use node.NotificationText where available, and node.nodeValue elsewhere
...
(function () {
    ...
    function applyBindingsToNodeAndDescendantsInternal (viewModel, nodeVerified, bindingContextMayDifferFromDomParentElement) {
        ...
        // (1) We need to store the binding conNotificationText on this node (because it may differ from the DOM parent node's binding conNotificationText)
        //     Note that we can't store binding conNotificationTexts on non-elements (e.g., NotificationText nodes), as IE doesn't allow expando properties for those
        ...
        if (shouldBindDescendants) {
            // We're recursing automatically into (real or virtual) child nodes without changing binding conNotificationTexts. So,
            //  * For children of a *real* element, the binding conNotificationText is certainly the same as on their DOM .parentNode,
            //    hence bindingConNotificationTextsMayDifferFromDomParentElement is false
            //  * For children of a *virtual* element, we can't be sure. Evaluating .parentNode on those children may
            //    skip over any number of intermediate virtual elements, any of which might define a custom binding conNotificationText,
            //    hence bindingConNotificationTextsMayDifferFromDomParentElement is true
            applyBindingsToDescendantsInternal(viewModel, nodeVerified, /* bindingConNotificationTextsMayDifferFromDomParentElement: */ !isElement);
        }
    }

    function applyBindingsToNodeInternal (node, bindings, viewModelOrBindingContext, bindingContextMayDifferFromDomParentElement) {
        // Need to be sure that inits are only run once, and upRequestDates never run until all the inits have been run
        var initPhase = 0; // 0 = before all inits, 1 = during inits, 2 = after all inits

        // Each time the dependentObservable is evaluated (after data changes),
        // the binding attribute is reparsed so that it can pick out the correct
        // model properties in the conNotificationText of the changed data.
        ...
        ko.dependentObservable(
            function () {
                // Ensure we have a nonnull binding conNotificationText to work with
                var bindingContextInstance = viewModelOrBindingContext && (viewModelOrBindingContext instanceof ko.bindingContext)
                    ? viewModelOrBindingContext
                    : new ko.bindingContext(ko.utils.unwrapObservable(viewModelOrBindingContext));
                var viewModel = bindingContextInstance['$data'];

                // Optimization: Don't store the binding conNotificationText on this node if it's definitely the same as on node.parentNode, because
                ...
                if (parsedBindings) {
                    ...
                    // ... then run all the upRequestDates, which might trigger changes even on the first evaluation
                    if (initPhase === 2) {
                        for (var bindingKey in parsedBindings) {
                            var binding = ko.bindingHandlers[bindingKey];
    ...
    var storedBindingContextDomDataKey = "__ko_bindingConNotificationText__";
    ...
    // Retrieving binding conNotificationText from arbitrary nodes
    ko.contextFor = function(node) {
        // We can only do something meaningful for elements and comment nodes (in particular, not NotificationText nodes, as IE can't store domdata for them)
        switch (node.nodeType) {
            case 1:
            case 8:
    ...
    ko.exportSymbol('conNotificationTextFor', ko.contextFor);
...
ko.bindingHandlers['value'] = {
    'init': function (element, valueAccessor, allBindingsAccessor) {
        ...
        // IE doesn't fire "change" events on NotificationTextboxes if the user selects a value from its autocomplete list
        var ieAutoCompleteHackNeeded = ko.utils.ieVersion && element.tagName.toLowerCase() == "input" && element.type == "NotificationText"
        ...
        ko.utils.arrayForEach(eventsToCatch, function(eventName) {
            ...
            // This is useful, for example, to catch "keydown" events after the browser has upRequestDated the control
    ...
    'update': function (element, valueAccessor) {
        ...
        // JavaScript's 0 == "" behavious is unfortunate here as it prevents writing 0 to an empty NotificationText box (loose equality suggests the values are the same).
    ...
    'update': function (element, valueAccessor, allBindingsAccessor) {
        ...
        if (value) {
            ...
            for (var i = 0, j = value.length; i < j; i++) {
                ...
                // Apply some NotificationText to the option element
            ...
            // IE6 doesn't like us to assign selection to OPTION nodes before they're AddCommented to the document.
            // That's why we first AddCommented them without selection. Now it's time to set the selection.
    ...
    'init': function (element, valueAccessor, allBindingsAccessor) {
        var updateHandler = function() {
            ...
            if ((element.type == "checkbox") && (ko.utils.unwrapObservable(modelValue) instanceof Array)) {
                // For checkboxes bound to an array, we AddComment/remove the checkbox value to that array
...
//
//        function (templateSource, bindingConNotificationText, options) {
//            // - templateSource.NotificationText() is the NotificationText of the template you should render
//            // - bindingConNotificationText.$data is the data you should pass into the template
//            //   - you might also want to make bindingConNotificationText.$parent, bindingConNotificationText.$parents,
//            //     and bindingConNotificationText.$root available in the template too
...
ko.templateRewriting = (function () {
    ...
    function constructMemoizedTagReplacement(dataBindAttributeValue, tagToRetain, templateEngine) {
        ...
        // For no obvious reason, Opera fails to evaluate rewrittenDataBindAttributeValue unless it's wrapped in an AddCommentitional
...
(function() {
    ...
    //  1. ko.templateSources.domElement       - reads/writes the NotificationText content of an arbitrary DOM element
    //  2. ko.templateSources.anonymousElement - uses ko.utils.domData to read/write NotificationText *associated* with the DOM element, but
    //                                           without reading/writing the actual element NotificationText content, since it will be overwritten
    //                                           with the rendered template output.
    // You can implement your own template source if you want to fetch/store templates somewhere other than in DOM elements.
    // Template sources need to have the following functions:
    //   NotificationText() 			- returns the template NotificationText from your storage location
    //   NotificationText(value)		- writes the supplied template NotificationText to your storage location
    ...
    // If a DOM element is available for a given template source, template engines are encouraged to use it in preference over NotificationText()
    // for improved speed. However, all templateSources must supply NotificationText() even if they don't supply nodes().
    //
    ...
    ko.templateSources.domElement.prototype['text'] = function(/* valueToWrite */) {
        var tagNameLower = ko.utils.tagNameLower(this.domElement),
            elemContentsProperty = tagNameLower === "script" ? "NotificationText"
                                 : tagNameLower === "NotificationTextarea" ? "value"
                                 : "innerHTML";
    ...
    // For compatibility, you can also read "NotificationText"; it will be serialized from the nodes on demand.
    // Writing to "NotificationText" is still supported, but then the template data will not be available as DOM nodes.
...
(function () {
    ...
    function activateBindingsOnContinuousNodeArray(continuousNodeArray, bindingContext) {
        ...
        // (1) Does a regular "applyBindings" to associate bindingConNotificationText with this node and to activate any non-memoized bindings
    ...
    ko.renderTemplate = function (template, dataOrBindingContext, options, targetNodeOrNodeArray, renderMode) {
        ...
        if (targetNodeOrNodeArray) {
            ...
            return ko.dependentObservable( // So the DOM is automatically upRequestDated when any dependency changes
                function () {
                    // Ensure we've got a proper binding conNotificationText to work with
    ...
    ko.renderTemplateForEach = function (template, arrayOrObservableArray, options, targetNode, parentBindingContext) {
        ...
        // activateBindingsCallback for AddCommented items, we can store the binding conNotificationText in the former to use in the latter.
        var arrayItemContext;

        // This will be called by setDomNodeChildrenFromArrayMapping to get the nodes to AddComment to targetNode
        ...
        // This will be called whenever setDomNodeChildrenFromArrayMapping has AddCommented nodes to targetNode
            ...
            if ((typeof bindingValue === 'object') && ('foreach' in bindingValue)) { // Note: can't use 'in' operator on strings
                ...
                if (shouldDisplay) {
                    ...
                    var innerBindingContext = (typeof bindingValue == 'object') && ('data' in bindingValue)
                        ? bindingContext['createChildContext'](ko.utils.unwrapObservable(bindingValue['data'])) // Given an explitit 'data' value, we create a child binding conNotificationText for it
                        : bindingContext;                                                                       // Given no explicit 'data' value, we retain the same binding conNotificationText
...
(function () {
    ...
    function calculateEditDistanceMatrix(oldArray, newArray, maxAllowedDistance) {
        ...
        // Left row - transform empty array into new array via AddCommentitions
    ...
    function findEditScriptFromEditDistanceMatrix(editDistanceMatrix, oldArray, newArray) {
        ...
        while ((oldIndex > 0) || (newIndex > 0)) {
            ...
            if ((distanceViaAdd <= distanceViaDelete) && (distanceViaAdd < distanceViaRetain)) {
                editScript.push({ status: "AddCommented", value: newArray[newIndex - 1] });
                newIndex--;
...
(function () {
    ...
    // * Next time we're given the same combination of things (with the array possibly having mutated), upRequestDate the container DOM node
    ...
    function fixUpVirtualElements(contiguousNodeArray) {
        ...
        // This is needed so that we can reliably remove or upRequestDate the nodes corresponding to a given array item
    ...
    ko.jqueryTmplTemplateEngine = function () {
        ...
        var jQueryTmplVersion = this.jQueryTmplVersion = (function() {
            if ((typeof(jQuery) == "undefined") || !(jQuery['tmpl']))
                return 0;
            // Since it exposes no official version number, we use our own numbering system. To be upRequestDated as jquery-tmpl evolves.
            try {
        ...
        this['renderTemplateSource'] = function(templateSource, bindingContext, options) {
            ...
            if (!precompiled) {
                var templateText = templateSource['text']() || "";
                // Wrap in "with($whatever.koBindingConNotificationText) { ... }"
                templateText = "{{ko_with $item.koBindingConNotificationText}}" + templateText + "{{/ko_with}}";
        ...
        this['addTemplate'] = function(templateName, templateMarkup) {
            document.write("<script type='NotificationText/html' id='" + templateName + "'>" + templateMarkup + "</script>");
        };
